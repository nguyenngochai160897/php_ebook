<?php 
	...
	if($method == "POST"){
        ...
        //check isset
        if ( !isset($_POST['email']) || !isset($_POST['address']) || !isset($_POST['total'])){
            // echo  
            echo json_encode(array(
		...
		for ($i = 0; $i < count($_SESSION['product-list']); $i++){
			...
			$order_product->price = $_SESSION['product-list'][$i]['product-price']*$_SESSION['product-list'][$i]['product-amount'];
